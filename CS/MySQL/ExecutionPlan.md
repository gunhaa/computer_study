...
- `ref`: Unique가 아닌 일반 인덱스(=) 조인 시 사용(`where a.index_col = 1`)
...
- `rows`: 쿼리 조건을 만족하는 행을 찾기 위해 MySQL이 조사해야 할 것으로 예상하는 레코드 건수 (실제 결과 건수가 아님, 실제로 읽는 수이기에 I/O에는 rows가 중요함)
  - 히스토그램의 버킷을 이용해 검색한다
...
- `Using filesort` (경고 - 튜닝 대상)
  ...
  - 조인에서 드라이빙 테이블을 이용한 정렬이며, 피드라이빙 테이블을 이용한 정렬이라면 `Using temporary; Using filesort`로 최악의 경우가 된다